...
const RegistInstitutionBK = ({navigation, route}: any) => {
  ...
  const onPressHandler = () => {
    ...
    const getInstituteByCode = async () => {
      await api
        ...
        .then(async ({data}) => {
          ...
          if (data === null) ToastAndroid.show('Kode sekolah tidak valid', ToastAndroid.LONG);
          else {
            setSchoolIsTrue(true);
            const group = data.groups.map(({id: value, name: label}: any) => ({value, label,}));
            setClassList(group);
            console.log('ini bawahnya', data.name)
    ...
    getInstituteByCode();
      ...
      navigation.navigate('RegistAccount', {full_name, role, institute_code, institute_name, groupIdSelected, institute_id});}
  };
  ...
  return (
    <ScrollView>
      ...
      <View style={{marginTop: schoolIsTrue ? 0 : 50, marginBottom: 30, alignItems: 'center'}}>
        <Image
      ...
      <View style={{margin: 20}}>
        ...
        <TouchableOpacity
          style={[Styles.button, {opacity: institute_code === '' || isLoading == true ? 0.5 : 1}]}
          onPress={onPressHandler}
          disabled={institute_code === '' || isLoading == true}>
